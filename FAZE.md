...
2026-05-26 - Dopuna event details/rating UX-a u tijeku: gornji detail summary vise ne prikazuje event rating nego organizer AVG rating, tagovi su u shared details summaryju bez `#` prefixa, lokacija otvara native maps i prikazuje udaljenost od korisnika, a puna adresa ostaje u detaljima. Prosli eventi se vise ne mogu naknadno joinati, a korisnik koji je bio prijavljen na prosli event moze ocijeniti organizatora.
...
2026-05-28 - Napravljeno: unauthenticated flow dobio je pocetni auth izbor s app ikonom i nazivom `Gdje i Kada`, odvojene Sign In i Sign Up screenove bez starog welcome carda, Google social login ispod glavnih auth akcija/formi i HR/EN i18n tekstove. Social auth logika je izdvojena u shared hook da se ne duplicira izmedu auth screenova. Backend auth endpointi ostaju isti jer `POST /api/auth/login`, `POST /api/auth/register`, `POST /api/auth/google` i `POST /api/auth/apple` vec pokrivaju trazeni flow. Test/build/typecheck nisu pokretani po dogovoru.

2026-05-28 - Napravljeno: rewrite kalendara u `Saved` tab. Postojeci calendar route sada rendera Saved ekran s `Collection` i `Schedule` segmentima; `Collection` prikazuje horizontalne lajkane event kartice, `Going soon` listu 3 najbliza joined eventa i `Past events` listu, a svaka sekcija ima zaseban `See all` ekran. `Schedule` zadrzava joined-only mjesecni kalendar, ali event oznake po danima su tockice. Backend je dobio `GET /api/users/me/saved-events/overview` koji server-side vraca preview sekcije za Saved. Dodani su HR/EN i18n kljucevi i dokumentiran je novi tab model. Datoteke: backend `EventController`, `EventService`, `SavedEventsOverviewDto`; frontend `app/(tabs)/calendar.tsx`, `app/saved/[section].tsx`, `features/saved/*`, calendar komponenta, API services/hooks/types, navigacija, i18n, `README.md`, `FAZE.md`. Testovi/build/typecheck nisu pokretani po dogovoru; pokrenut je samo `git diff --check`.

2026-05-28 - Napravljeno: dorada event tagova i map filtera. Create event vise nema slobodan unos tagova nego koristi searchable katalog u 5 kategorija s limitom 2 taga po kategoriji; backend validira isti katalog i odbija nepoznate tagove. `GET /api/events` prima `tags` i `attendanceModes`, uvijek clamp-a discovery na buducnost, a map search pokriva naslov, lokaciju, adresu i tagove. Search bar na mapi dobio je filter gumb s modalom za datum/tagove, ispod su horizontalni quick filteri `Danas/Tonight`, `This week`, `Free`, `Paid`, `Reservation`, `Weekend`. Shared details tag cell prikazuje prvi tag bez `#` i na tap se na 5 sekundi mijenja u vertikalni scroll svih tagova. Datoteke: backend `EventController`, `EventService`, `EventTagCatalog`, `EventMapper`; frontend `core/events/event-tags`, create tag selector, map filter modal/search/quick filters, shared details, i18n, README/FAZE. Testovi/build/typecheck nisu pokretani po dogovoru.

2026-05-30 - Napravljeno: profil i account management dorada. Inbox unread badge sada koristi postojeći purple `mapAccent`; profil meni ide redom Lajkani eventi, Moje ulaznice, Povijest aktivnosti, Upravljanje eventima, Postavke; edit profil koristi gallery upload avatara do 5 MB uz HEIC->PNG pripremu preko postojeceg image normalizera. Backend je dobio avatar storage na S3-compatible sloju i authenticated `/api/users/{id}/avatar/content`, Google prvi social signup sprema Google `picture` kao inicijalni avatar URL, a Apple path ostaje kompatibilan bez obaveznog avatara. Settings imaju promjenu lozinke i brisanje profila: brisanje odmah anonimizira javni identitet u `Deleted user`, uklanja osobne postavke/push/friend/feed podatke i uploaded avatar, zadrzava public/private poruke, recenzije, transakcije i evente, te sprema recovery podatke 30 dana u `account_deletion_recoveries`; ponovna prijava unutar 30 dana automatski ponistava brisanje, nakon isteka scheduled purge i auth flow ciste recovery i social identity podatke. Datoteke: backend `V19__profile_avatar_password_account_deletion.sql`, `AuthService`, `JwtAuthenticationFilter`, `UserController`, `ProfileService`, `AuthMapper`; frontend profile/settings screens, API services/hooks/types, avatar component, i18n, README/FAZE. Testovi/build/typecheck nisu pokretani po dogovoru.

2026-05-30 - Napravljeno: settings navigacija je spljostena. `app/profile/settings.tsx` sada direktno prikazuje Jezik, Tema, Radijus, Obavijesti, FYP postavke i Racun bez dodatnog `Postavke/Preferences` meduekrana; promjena lozinke, brisanje profila i odjava su premjesteni u novi `app/profile/account.tsx`. Odabrani jezik i tema imaju purple highlight vidljiv i na light modeu. Testovi/build/typecheck nisu pokretani po dogovoru; pokrenut je samo `git diff --check`.

2026-05-30 - Napravljeno: Discover/FYP redizajn prema novom full-screen media stilu. Feed header sada ima preset tabove `For you`, `Tonight`, `Weekend`, `Trending`, `Friends`, filter gumb i chipove za spremljenu FYP lokaciju/tip eventa; filter se persistira u Zustand storeu i podrzava trenutnu lokaciju/radius, grad ili drzavu. Backend `GET /api/feed` ostaje kompatibilan, ali prima opcionalne `from/to`, `lat/lng/radiusKm`, `query`, `tags`, `attendanceModes`, `scope` i `sort` parametre za Discover presete. Reels media ostaje video-prvi pa horizontalne slike, vertikalni event snap ostaje kroz paginirani `FlatList`, a donji tekst je bez kartice na transparent-to-black scrimu radi kontrasta. Datoteke: backend `EventController`, `EventService`, `EventMapper`, `EventMapper.xml`, `README.md`; frontend `app/(tabs)/fyp.tsx`, `core/store/app-store.ts`, `core/types/domain.ts`, `core/api/query-hooks.ts`, `core/api/query-keys.ts`, `features/events/fyp/fyp-feed-filters.ts`, `features/events/components/fyp/*`, i18n, README/FAZE. Testovi/build/typecheck nisu pokretani po dogovoru.
...
2026-06-01 - Napravljeno: Discover/FYP summary overlay prikazuje vrijeme, sudionike i adresu u tamnim meta bubbleima. Event tagovi ostaju u zasebnom purple bubble redu ispod meta bubblea; prikazuje se najvise 5 tagova, a sesti bubble prikazuje overflow u obliku `+N`. Uz `DETAILS` je dodan chevron-down gumb koji spusta overlay u compact stanje s datumom/statusom i naslovom eventa; prvi tap na compact overlay samo ga ponovno prosiri, a iduci tap opet otvara detalje. Backend nije mijenjan jer feed DTO vec vraca `address` i `tags`. Datoteke: `features/events/components/fyp/fyp-reel-summary-card.tsx`, `core/i18n/translations.ts`, `README.md`, `FAZE.md`. Testovi/build/typecheck nisu pokretani po dogovoru.

2026-06-01 - Napravljeno: map search filter modal na `Date` tabu vise ne prikazuje quick preset gumbe `Today`, `Tomorrow` i `This week`; ostaje samo `All dates`, a konkretan dan ili raspon bira se kroz kalendar. Vanjski horizontalni quick filteri na mapi (`Tonight/This week/Free/Paid/Reservation/Weekend`) nisu dirani. Backend nije mijenjan jer `/api/events` vec prima isti `from/to` model. Datoteke: `features/events/components/map-filter-modal.tsx`, `README.md`, `FAZE.md`. Testovi/build/typecheck nisu pokretani po dogovoru.
...
2026-06-02 - Napravljeno: regresije FYP sharea, vlastitih in-app chat notifikacija i poll vote synca. Event share modal sada zatvara transparentni RN modal prije native share sheeta da se nakon izlaska iz sharea ne ostavi touch-capturing overlay. Chat realtime listener za `message.created` prije in-app modala dohvaća konkretnu poruku i preskače modal ako je `message.mine`, dok backend push mapper već isključuje sendera. `poll.updated` websocket event sada radi full chat room sync umjesto inkrementalnog mergea preko SQLite cachea, jer poll vote mijenja postojeću poll poruku bez novog message id-ja. Backend nije mijenjan jer su push sender filter i poll vote insert već bili ispravni; regresije su bile u frontend modal/realtime/cache sloju. Datoteke: `features/events/components/event-share-modal.tsx`, `core/api/chat-realtime-listener.tsx`, `core/api/services.ts`, `README.md`, `FAZE.md`. Testovi/build/typecheck nisu pokretani po dogovoru.

2026-06-02 - Napravljeno: popravak grupiranja map pinova za evente na istoj adresi. Grouping key je izdvojen u `components/map/event-map-grouping.ts` i sada koristi canonical `street + city + postal` iz `address` vrijednosti kad postoji, bez location namea i bez uvjeta da su koordinate identicne do 6 decimala. Time se grupiraju i isti venueovi ciji backend address format varira izmedu `City of Zagreb/Croatia` i `Grad Zagreb/Hrvatska`. Ako adresa nedostaje, zadrzan je coordinate/location fallback. Grupirani pinovi prikazuju `+N` count badge i marker tap otvara postojeći selection sheet s eventima iz grupe. Datoteke: `components/map/event-map.tsx`, `components/map/event-map-grouping.ts`, `components/map/map-marker-badge.tsx`, `README.md`, `FAZE.md`. Testovi/build/typecheck nisu pokretani po dogovoru.

2026-06-02 - Napravljeno: dorada grouped-pin selection sheeta i nearby grid metapodataka. `MapNearbySheet` dobio je `variant="selection"` za grupirane pinove: header subtitle prikazuje udaljenost zajednickog venuea s location ikonom, cardovi u gridu ne prikazuju distance nego compact broj dana do eventa (`Nd`), a povlacenje prema dolje zatvara sheet skroz bez preview/nearby medustanja. Normalni expanded nearby/city pregled zadrzava distance i ispod nje dodaje isti `Nd` label u cardu. Event cardovi za `joined/approved` evente dobivaju suptilni zeleni border/glow bez bojanja unutrasnjosti carda. Datoteke: `features/events/components/map-nearby-sheet.tsx`, `features/events/components/events-map-experience.tsx`, `README.md`, `FAZE.md`. Testovi/build/typecheck nisu pokretani po dogovoru.

2026-06-02 - Napravljeno: redizajn shared event detailsa i ticket checkout entryja. Direct details screen vise nema card border ni donje tekst akcije nego icon-only back/share header. `EventDetailsContent` sada u mapi, Discoveru i direct details screenu prikazuje poster/naslov/datum, purple CTA, horizontalne tagove s `+N` prosirenjem, summary s ocjenom eventa/polaznicima/cijenom/udaljenosti i clickable `Adresa` rowom koji otvara native maps, klikabilnu galeriju s fullscreen paging + zoom previewom, opis, organizer blok i standardne detalje. Paid CTA vodi na novi `app/tickets/checkout.tsx` checkout ekran koji koristi postojece backend endpointove `ticket-checkout` i `ticket-orders/{id}/confirm`, a free/open i waitlist CTA ostaju na postojecem join flowu bez placanja, s tekstovima `Pridruzi se` i `Prijavi se`. Backend model nije mijenjan jer DTO vec vraca cijenu, valutu, tagove, ratinge i organizer podatke. Datoteke: `features/events/components/event-details-content.tsx`, `components/events/event-image-preview-modal.tsx`, `features/events/hooks/use-event-join-actions.ts`, `app/tickets/checkout.tsx`, `app/_layout.tsx`, `core/i18n/translations.ts`, `README.md`, `FAZE.md`. Testovi/build/typecheck nisu pokretani po dogovoru.
...
2026-05-22 - Dopuna Google auth backend dijagnostike: backend sada razlikuje neispravan Google/Apple token od situacije kad backend container ne moze dohvatiti provider JWKS certifikate (`www.googleapis.com`/`appleid.apple.com`). Za provider network/DNS problem vraca `503` s jasnom API `message`, a globalni `ResponseStatusException` handler vraca strukturirani error JSON koji frontend vec cita. Backend README dodaje Podman dijagnostiku za Google JWKS DNS/egress problem. Datoteke: backend `ApiErrorResponse`, `ApiExceptionHandler`, `OAuthProviderAvailability`, Google/Apple verifieri, `README.md`, `FAZE.md`. Testirano: nisu pokretani testovi, typecheck ni build po dogovoru; samo staticka provjera diffa.

2026-05-22 - Dopuna Android push registracije: maknuta je pogresna provjera `Constants.isDevice` jer taj property nije dio trenutnog `expo-constants` runtimea, pa je Android test build uvijek zavrsavao kao `unavailable` prije dohvacanja Expo push tokena. Frontend sada odbija samo web i Android Expo Go runtime, a EAS development/test/internal/production buildovi pokusavaju normalan `getExpoPushTokenAsync({ projectId })` flow. `Preferences > Notifications` prikazuje odvojene lokalizirane poruke za web, Expo Go runtime, missing `projectId`, permission denied i stvarni native/FCM error. Datoteke: `core/notifications/push-notifications.ts`, `app/profile/preferences/notifications.tsx`, `core/i18n/translations.ts`, `README.md`, `FAZE.md`. Test/build/typecheck nisu pokretani po dogovoru; napravljena je staticka provjera diffa i referenci.

2026-05-22 - Dopuna Android Firebase wiringa za push: EAS test build ostaje managed/prebuild build bez committanog `android/` foldera. `app.config.js` sada za test build faila rano ako nema Firebase app configa i prihvaca EAS file env var `GOOGLE_SERVICES_JSON`, `GOOGLE_SERVICES_JSON_PATH`, root `google-services.json` ili lokalni native `android/app/src/qa/google-services.json`; `test` EAS profile koristi `environment: "preview"` radi tog file secreta. Dokumentirano je da EAS FCM V1 service account key i Firebase `google-services.json` nisu isti file: admin/service-account JSON ostaje u EAS credentials za slanje push poruka, a `google-services.json` za package `com.anonymous.GdjeIKadaNative.test` mora biti dostupan build procesu da APK inicijalizira Firebase na uredaju. `.gitignore` opet ignorira cijeli generated `android/` folder da EAS ne pokusa bare build bez `android/gradlew`. Datoteke: `.gitignore`, `app.config.js`, `eas.json`, `README.md`, `TEST_BUILD.md`, `FAZE.md`. Test/build/typecheck nisu pokretani po dogovoru; napravljena je staticka provjera diffa i referenci.
...
- Eventi imaju katalogizirane tagove koji se spremaju na backend i prikazuju u feed/detail ekranima; aktualni create flow dopusta najvise 2 taga po kategoriji.
...
2026-05-27 - Dopuna map/navigation redesigna: glavni tab layout sada koristi custom floating toolbar s redoslijedom Mapa, Discover, Kalendar, Inbox i Profil; social ruta ostaje sekundarna, a create event modal otvara floating `+` na mapi. Mapa dobiva nearby sheet koji se otvara pri ulasku, zatvara povlacenjem/tapom u handle, ostavlja pull handle iznad toolbara, ima horizontalne event cardove, `See all` grid i like akciju; nearby radius je persistiran u app storeu i mijenja se u `Preferences` od 1 do 20 km. Map pinovi i nearby cardovi koriste centralne theme boje za TODAY/SUTRA/THIS WEEK badgeve, a map pinovi se grupiraju samo za potpuno istu adresu/koordinatu i tada otvaraju selection sheet prije detalja. Chat realtime listener prikazuje in-app modal za novu poruku dok je app aktivan, Inbox tab prikazuje purple unread dot, foreground push banner se suzbija, a backend `NotificationService` preskace Expo push za primatelje s otvorenom websocket sesijom. Datoteke: frontend `app/(tabs)/_layout.tsx`, `components/navigation/android-floating-tab-bar.tsx`, `features/events/components/events-map-experience.tsx`, `features/events/components/map-nearby-sheet.tsx`, `components/map/*`, `core/store/app-store.ts`, `core/theme/*`, `core/i18n/translations.ts`, `core/api/chat-realtime-listener.tsx`, `core/notifications/push-notifications.ts`, `app/profile/preferences.tsx`; backend `ChatRealtimeSessionRegistry`, `NotificationService`; dokumentacija `README.md`, `FAZE.md`. Testirano: nisu pokretani testovi, typecheck ni build po dogovoru; napravljena je staticka provjera citanjem i `rg` referenci.

2026-05-27 - Dopuna nakon prve provjere redesigna: iOS je vracen na NativeTabs/Liquid Glass toolbar, srednji tab je kalendar, Android FYP koristi full-screen viewport i dize summary/actions iznad floating toolbara, nearby pocetno stanje je manji horizontalni preview s compact karticama i `See all` otvara odvojeni 2-column scroll prikaz bez pull-up geste. Nearby grad iz event adrese vise ne uzima postanski broj za Zagreb, a map time badge je podignut iznad event ikonice. Testirano: nisu pokretani testovi, typecheck ni build po dogovoru; samo `git diff --check`.

2026-05-27 - Dopuna nearby/map marker finese: `See all` u nearby sheetu sada prikazuje ucitane eventove iz istog grada sortiranih po udaljenosti, dok pocetni preview ostaje radius-based. Map marker date tag izdvojen je u zaseban subcomponent i pozicioniran kao vidljiv pill preko donjeg ruba kruzne event slike s dotom odmah ispod. Testirano: nisu pokretani testovi, typecheck ni build po dogovoru; samo `git diff --check`.

2026-05-30 - Dopuna Lexend font konzistencije: frontend vise ne ostavlja native `TextInput`, `react-native-calendars` ili `react-native-ui-datepicker` tekstove na system fallbacku. Direktni search/chat/calendar inputi dobili su `theme.tokens.typography.body.fontFamily`, calendar theme sada eksplicitno postavlja Lexend font family vrijednosti, date filteri i date/time picker koriste Lexend style objekte, a `AppDateTimeField` je prebacen na JS picker da date/time odabir ostane pod app font sustavom. Datoteke: `components/primitives/app-date-time-field.tsx`, `components/search/map-search-bar.ios.tsx`, `features/calendar/components/joined-events-calendar.tsx`, `features/events/components/map-date-filter-control.tsx`, `features/events/components/map-filter-modal.tsx`, `app/chat/[id].tsx`, `app/(tabs)/messages.tsx`, `README.md`, `FAZE.md`. Testirano: `npm run typecheck`, `git diff --check` i staticka `rg` provjera font referenci.

...
- Profil je implementiran kao `app/(tabs)/profile.tsx`; sada je activity/profile hub s avatarom, imenom, bio tekstom, edit profilom, liked historyjem, Moje ulaznice, activity historyjem, event managementom i odvojenim settings screenom.
...
- Data fetching/cache: TanStack React Query u `core/api/query-hooks.ts` i `core/query/query-client.ts`, uz per-user Expo SQLite lokalni cache u `core/cache/local-cache.ts` za event details, chat room metadata i chat poruke. Private event videi imaju zaseban `FileSystem.cacheDirectory` cache u `core/cache/video-cache.ts` s 350 MB LRU limitom i cache keyem vezanim uz `mediaId` + `event.cacheVersion`.
...
- Font: Lexend se ucitava kroz `expo-font` iz `assets/fonts`. `AppText`, `AppInput`, direktni `TextInput` usagei, tab labeli, `react-native-calendars` theme i `react-native-ui-datepicker` style objekti moraju eksplicitno koristiti Lexend tokene; ne oslanjamo se na system fallback font.
...
- `GET /api/locations/search?query=&locale=&limit=&lat=&lng=&types=`
- `GET /api/events/{id}`
- `GET /api/events/{id}/cache-state`
- `POST /api/events`
- `PATCH /api/events/{id}`
...
- `POST /api/events/{id}/media/video` raw binary video upload
- `GET /api/events/{id}/media/{mediaId}/content`, video-friendly `/content/{filename}` i image thumbnail `/thumbnail`
...
- `PATCH /api/users/me/profile` multipart `profile` JSON + `avatar`
- `GET /api/users/{userId}/avatar/content`
...
- `GET /api/feed?cursor=&limit=&seed=&excludeEventIds=&from=&to=&lat=&lng=&radiusKm=&query=&tags=&attendanceModes=&scope=&sort=`
...
- `GET /api/messages/chat-rooms/{id}?afterMessageId=&limit=`
- `PATCH /api/messages/chat-rooms/{id}`
- `PATCH /api/messages/chat-rooms/{id}/notification-settings`
- `GET /api/messages/chat-rooms/{id}/messages?afterMessageId=&limit=`
...
- `app_users` iz `V3__create_users_table.sql`
  ...
  - avatar storage metadata, `account_status`, `deletion_requested_at`, `deletion_scheduled_at`, `deleted_at` iz `V19__profile_avatar_password_account_deletion.sql`
  - `created_at`, `updated_at`
- `account_deletion_recoveries` iz `V19__profile_avatar_password_account_deletion.sql`
...
Event trenutno podrzava naslov, lokaciju, adresu, opis, start/end datum, coordinates, entrance coordinates, entry instructions, katalogizirane tagove iz 5 kategorija s najvise 2 taga po kategoriji, creator user id, creator name/avatar za organizer prikaz, visibility `public/friends`, attendance mode `open/waitlist/paid`, cijenu za paid evente, capacity, status, `sourceUrl` za vanjske/importirane evente, `updatedAt`, `cacheVersion`, event i organizer rating agregate, `likeCount`, `likedByMe`, participant count, `waitlistCount`, `joinedByMe`, `attendanceStatus` i `canJoin`. Friends-only eventi se na mapi, FYP-u, detailsu i authenticated media endpointu vide samo kreatoru i prihvacenim prijateljima kreatora; neprijatelji ne dobivaju ni event ni media. Create flow u frontendu koristi jedan naziv/lokaciju/opis bez odvojenih HR/EN polja, a backend `CreateEventRequest` prihvaca canonical `title`, `where`, `about`, `entryInstructions` i `tags` te ih mirror-a u postojece HR/EN stupce radi kompatibilnosti. Adresa u create flowu bira se kroz backend `GET /api/locations/search` autocomplete; odabrani rezultat postaje canonical event coordinate, a entrance pin je opcionalna zasebna koordinata. Step 5 u create flowu je obavezan media korak: korisnik mora dodati barem jednu JPG/PNG sliku, najvise 5 slika, svaka do 5 MB i barem 640x640 px; opcionalno moze dodati jedan MP4/MOV/M4V video do 10 MB. Frontend kreira event multipart requestom s `event` JSON-om i obaveznim `images`, a opcionalni video odmah nakon toga salje kao raw binary upload kroz `expo-file-system` na `/api/events/{id}/media/video`, bez RN `FormData` multiparta za video. Backend sprema media kroz S3-compatible storage provider i `event_media` metadata ukljucujuci originalni naziv filea, originalnu sliku i backend-generirani private thumbnail do 600x400 za map/list prikaze. Uploaded media URL-ovi u DTO-u pokazuju na authenticated backend endpoint `/api/events/{eventId}/media/{mediaId}/content`, a thumbnail URL na `/api/events/{eventId}/media/{mediaId}/thumbnail`; za video backend moze vratiti i `/content/{filename}` varijantu s ekstenzijom radi iOS AVPlayer detekcije. Frontend centralno dodaje Bearer header za prikaz slika i videa, pa friends-only media ne treba public MinIO bucket. External URL media ostaje legacy/public-only put i ne dopusta se za friends-only evente. Video se ne prikazuje na mapi ni event bubbleima; Discover/FYP prikazuje video kao prvu stranicu media pagera ako postoji, a slike su dostupne horizontalnim scrollom. Private video prvi put streama authenticated URL i u pozadini se sprema u `FileSystem.cacheDirectory`; iduci put `expo-video` dobiva lokalni `file://` URI, cache se verzionira po `mediaId` + `event.cacheVersion`, ogranicen je na 350 MB po LRU pravilu i brise se za prethodnog korisnika kod user switch/logout toka. Discover feed sada prima persistirani filter iz Zustand storea: trenutna lokacija koristi `lat/lng/radiusKm`, grad ili drzava biraju se kroz Nominatim city/country autocomplete, spremaju se tek nakon klika na prijedlog i samo tada salju kao `query`, preset `Friends` salje `scope=friends`, `Trending` salje `sort=trending`, a `Tonight/Weekend` salju `from/to`. Kad se svi rezultati filtriranog Discover feeda potrose, frontend za sve presete osim `Tonight` i `Weekend` automatski ukljucuje opusteni fallback feed po 2 eventa i backendu salje `excludeEventIds` da ne ponovi vec prikazane filtrirane evente. Video se pauzira kad event nije aktivan ili kad korisnik ode na sliku, jedan tap po sredini Discovera toggla globalni mute za feed, a double tap samo lajka event. Vanjski source URL se ne lijepi u opis; detail UI ga prikazuje skroz na dnu kao `Izvor`/`Open page` akciju bez ispisivanja linka. Shared detail header sada ima poster, naslov, datum/vrijeme, purple CTA (`Kupi ulaznicu`, `Pridruzi se` ili `Prijavi se` ovisno o attendance modeu), horizontalni tag strip s `+N` prosirenjem i summary red: ocjena eventa, polaznici, cijena (`Besplatno`/`Free` za open/waitlist bez cijene) i udaljenost koja otvara native maps. Slike u detail galleryju otvaraju fullscreen preview s horizontalnim pagingom, pinch zoomom, panom i double-tap zoomom. Ispod opisa je organizer blok s avatarom, imenom, AVG ocjenom i brojem recenzija, a zatim standardni detail rowovi. Create i owner manage screen imaju fullscreen image preview s pinch/double-tap zoomom te odvojene video i image media sekcije. Vlasnik eventa kroz profil ima Created Events ekran i manage ekran za izmjenu osnovnih detalja, upload/brisanje videa i slika, pregled/prihvacanje waitliste, micanje sudionika i blokiranje korisnika s neplacenih eventova. Backend za to koristi owner-only endpointove `PATCH/DELETE /api/events/{id}`, media endpointove, participant endpointove, `event_blocks` i in-app `app_notifications`. Join/leave i like/unlike rade kroz backend, a join je onemogucen za prosle evente. Feed i detail endpointi vracaju `event_media` za reels/detail prikaz, a frontend sprema dobivene evente u SQLite i pri direct details otvaranju prvo provjerava `/cache-state`. FYP `Not interested` preference spremaju se server-side po eventu, kreatoru ili tagu i filtriraju discovery za tog korisnika; tag akcije se prikazuju bez hashtag prefixa. FYP impressions se spremaju po korisniku/eventu kroz `event_feed_impressions`; eventi koji su se vise puta prikazali bez interakcije padaju nize u feed rankingu, a like/join/share/not-interested oznacavaju interakciju. Korisnik koji je bio prijavljen na prosli/zavrseni event moze ocijeniti organizatora, a taj AVG rating se prikazuje u organizer bloku event detailsa. Paid join iz details CTA-a otvara `app/tickets/checkout.tsx`: ekran koristi postojece `POST /api/events/{eventId}/ticket-checkout` i `POST /api/ticket-orders/{orderId}/confirm`, prikazuje Apple Pay ili Google Pay entry point prema platformi, zapisuje transaction kroz backend stub i tek tada join-a event. Free/open i waitlist CTA ostaju na postojecem join flowu bez placanja, s tekstovima `Pridruzi se` i `Prijavi se`. Real Stripe React Native SDK/PaymentSheet nije dodan jer ova promjena ne pokrece native build.
...
Dopuna 2026-06-02: Shared event details header redizajniran je prema ticket-first layoutu: direct details stranica ima icon-only back lijevo i share desno bez card bordera ili donjih tekst akcija; poster/naslov/datum ostaju na vrhu, ispod je purple CTA, horizontalni tag strip s `+N` prosirenjem, summary red s ocjenom eventa, polaznicima, cijenom i udaljenosti, a `Adresa` detail row otvara native maps, fullscreen image gallery s pagingom/zoomom, opis, organizer blok i zatim detalji. Paid CTA vodi na novi `app/tickets/checkout.tsx` checkout screen s Apple Pay/Google Pay entry pointom preko postojeceg backend checkout stuba; free/open i waitlist CTA ostaju na postojecem join flowu bez placanja, s tekstovima `Pridruzi se` i `Prijavi se`.
...
FYP dio aplikacije sluzi za brz discovery kroz vertikalni Reels-style feed. Svaki reel je povezan s eventom. Korisnik moze lajkati, shareati i otvoriti detalje eventa. Feed se randomizira seed vrijednoscu pri ulasku u FYP, prikazuje samo buduce published evente dostupne korisniku, koristi paginirani buffer umjesto ucitavanja svih eventova odjednom i filtrira se kroz spremljeni Discover filter. Default je trenutna lokacija/radius, a korisnik moze spremiti grad ili drzavu odabranu iz OpenStreetMap/Nominatim autocompletea kao FYP lokaciju te koristiti presete `For you`, `Tonight`, `Weekend`, `Trending` i `Friends`.
Nakon kraja filtriranog feeda Discover za `For you`, `Trending` i `Friends` nastavlja s opustenim fallback feedom po 2 eventa. Fallback i dalje postuje access, future-only, published status, blocks i Not interested preference, ali vise nije vezan uz odabranu lokaciju/tip/preset. `Tonight` i `Weekend` nemaju fallback jer moraju ostati vremenski striktni. Feed cursor nosi vec vracene event id-jeve kako `POST /api/feed/impressions` ne bi promjenom rankinga vratio duplikat na iducoj stranici, a frontend dodatno dedupe-a evente po `id`.
...
Poruke podrzavaju privatne razgovore, grupe, event-specific grupe, pollove, friend request poruke u direct chatu i admin-only chat mod gdje samo admini pisu, a ostali mogu glasati na pollu. Nove text poruke se na backendu spremaju encrypted-at-rest AES-GCM-om; `MESSAGES_ENCRYPTION_SECRET` je preporuceni env secret, a lokalni fallback je postojeci JWT secret radi kompatibilnosti. Slanje poruke sada nakon spremanja na backendu salje Expo push notifikaciju svim ostalim clanovima sobe ako korisnik ima registriran push token, ukljucene preference za taj tip chata i nije muteao tu sobu. Frontend per-user sprema chat room metadata i poruke u Expo SQLite; ulazak u chat salje zadnji lokalno spremljeni `afterMessageId`, pa backend vraca samo nove poruke umjesto cijelog historyja.

Profil prikazuje korisnikovu aktivnost i meni redom: Lajkani eventi, Moje ulaznice, Povijest aktivnosti, Upravljanje eventima i Postavke. Profil ima editiranje imena, bio teksta i upload profilne slike do 5 MB; iOS HEIC se na frontendu priprema kao PNG prije uploada. Postavke su odvojeni ekran otvoren iz profila; tamo su direktno jezik, tema, radijus mape, obavijesti, FYP postavke i Racun. Racun je zaseban ekran za promjenu lozinke, brisanje profila i odjavu. Brisanje profila odmah anonimizira javni identitet u `Deleted user`, uklanja osobne detalje/avatar/push i preference podatke, ali zadrzava public evente, recenzije, poruke i history bez osobnih podataka. Recovery je moguc 30 dana automatski ponovnom prijavom, nakon cega backend scheduled purge brise recovery i social identity podatke. `Settings > Notifications` trenutno sadrzi push permission/registration CTA te odvojene togglove za privatne poruke i grupne/event chatove. `Settings > FYP preferences` prikazuje `Not interested` stavke grupirane po eventima, kreatorima i tagovima.
...
- Query: `useFeedInfiniteQuery()` prema `GET /api/feed?cursor=&limit=&seed=&excludeEventIds=&from=&to=&lat=&lng=&radiusKm=&query=&tags=&attendanceModes=&scope=&sort=`.
- Media helpers: `core/events/event-cover.ts`.
- Reels slide UI: public compatibility export `features/events/components/fyp-reel-slide.tsx`, stvarna implementacija u `features/events/components/fyp/` (`fyp-discover-header`, `fyp-reel-slide`, summary, action rail, video layer, layout helpers) i filter helper u `features/events/fyp/fyp-feed-filters.ts`.
...
- Video playback: `expo-video` (`VideoView`, `useVideoPlayer`) s preload prozorom oko aktivnog itema; ako trenutni native build nema `ExpoVideo` modul, FYP ostaje na poster fallbacku bez crasha dok se ne rebuilda app. Private video cache je u `core/cache/video-cache.ts`: player odmah moze streamati remote authenticated URL, a background download sprema lokalni file za iduce gledanje.

Trenutno ponasanje:

- Vertikalni `FlatList` s `pagingEnabled`, `snapToInterval`, `onViewableItemsChanged` i `fetchNextPage()`.
- Feed je server-side paginiran cursorom i vraca `items`, `nextCursor`, `hasMore`; cursor ukljucuje vec vracene event id-jeve da iduce stranice ne ponavljaju evente i kad se impression ranking promijeni tokom scrolla. Frontend koristi page size 8 i `FlatList` preload prozor oko aktivnog reela da ne ucitava sve evente odjednom.
- Feed se randomizira novim seed vrijednostima pri ulasku u FYP, prikazuje samo buduce published evente dostupne korisniku i salje `POST /api/feed/impressions` kad reel postane aktivan. `For you` koristi spremljenu FYP lokaciju, `Tonight` i `Weekend` dodaju date range, `Trending` sortira po sudionicima/lajkovima, a `Friends` ogranicava feed na evente kreatora koji su prihvaceni prijatelji korisnika. Kad primarni feed vise nema stranica, `For you`, `Trending` i `Friends` otvaraju fallback query s `limit=2` i `excludeEventIds`, dok `Tonight` i `Weekend` ostaju fiksni.
...
- Details iz FYP-a otvaraju FYP-specific bottom sheet unutar native modala, s handle barom i scrollable `EventDetailsContent`; modal ide preko native tab bara, a Android modal root je wrapan u `GestureHandlerRootView` radi stabilnog drag-down gesturea. Donji summary/CTA je tappable. Map screen i dalje koristi map `EventDetailSheet`.
...
- Media upload/create flow je povezan s `event_media` CRUD-om za slike i opcionalni jedan video do 10 MB; create flow salje slike u create multipart requestu, a video nakon kreiranja kao raw binary owner media upload kroz `expo-file-system`.
...
- Mjesecni grid je implementiran kroz `features/calendar/components/joined-events-calendar.tsx` i `react-native-calendars`, uz eksplicitne Lexend `textDayFontFamily`, `textMonthFontFamily` i `textDayHeaderFontFamily` theme vrijednosti.
...
- Edit profila je u `app/profile/edit.tsx` i sprema ime, bio i avatar upload preko `PATCH /api/users/me/profile`; multipart verzija salje `profile` JSON i `avatar` file.
...
- Moje ulaznice su u `app/profile/transactions.tsx` i koriste `GET /api/users/me/transactions`.
...
- Real Stripe RN PaymentSheet integracija kad se predje sa stub checkouta na produkcijska placanja.
...
- `GET /api/events?from=&to=&lat=&lng=&radiusKm=&query=&tags=&attendanceModes=` -> `EventService.getEvents(...)`
- `GET /api/feed?cursor=&limit=&seed=&excludeEventIds=&from=&to=&lat=&lng=&radiusKm=&query=&tags=&attendanceModes=&scope=&sort=` -> `EventService.getFeed(...)`
...
- `POST /api/events/{id}/media`, `POST /api/events/{id}/media/video` i `DELETE /api/events/{id}/media/{mediaId}` -> owner-only media management
- `GET /api/events/{id}/media/{mediaId}/content` -> authenticated media stream koji koristi isti event access check kao event details
...
- Parsira/validira feed `cursor`, `limit`, `seed` i `excludeEventIds`, filtrira samo buduce published evente dostupne korisniku, randomizira redoslijed po seedu, spusta evente s vise neinteraktiranih FYP prikaza nize u listi i u `nextCursor` sprema vec vracene evente radi feed-level dedupe-a.
...
- Trenutno stanje: implementiran je Stripe-named stub provider kroz `PaymentProvider`/`StripePaymentProvider`, bez `@stripe/stripe-react-native` dependencyja i bez native build promjena. Stub vraca `clientSecret`, frontend prikazuje `app/tickets/checkout.tsx` checkout ekran s Apple Pay/Google Pay entry pointom i backend zapisuje `ticket_orders`, `payments` i `transactions`.
...
- Android testiranje: push se moze testirati na pravom Android uredaju u instaliranom EAS test/internal APK-u nakon sto `Settings > Notifications` vrati Expo push token i backend spremi token. Ako registracija padne, ekran prikazuje konkretan native/FCM error uz lokaliziranu poruku umjesto genericke "wrong device/build" poruke.
